...
export class ItemMapper {
  ...
  static reverseTransform(itemsV2: any[]): any[] {
    ...
    return itemsV2.map((item) => ({
      ...item, // Retain any other properties that are not explicitly transformed
      fulfillment_id: Array.isArray(item.fulfillment_ids) ? item.fulfillment_ids[0] : undefined, // Convert array to single value
      category_id: Array.isArray(item.category_ids) ? item.category_ids[0] : undefined, // Convert array to single value
